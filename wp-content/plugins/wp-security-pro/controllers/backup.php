 ...
 if(current_user_can( 'manage_options' ) && isset($_POST['option']))
		{
	        switch($_POST['option'])
			{
				case "mo2f_enable_cron_backup":
					mo_wpns_handle_db_enable_form($_POST);				        break;
				case "mo2f_cron_backup_configuration":
					mo_wpns_handle_db_configuration_form($_POST);			    break;
				case "mo2f_enable_cron_file_backup":
					mo_wpns_handle_file_backup_enable_form($_POST);			break;
			}
		} 
    ...
    function mo_wpns_handle_db_enable_form($postData){
		...
		if(get_option('mo2f_enable_cron_backup') == '0'){
            ...
            do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('CRON_DB_BACKUP_DISABLE'),'ERROR');
            }else{
             do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('CRON_DB_BACKUP_ENABLE'),'SUCCESS');
            }
    }

    function mo_wpns_handle_db_configuration_form($postData){
        $mo2f_cron_hours = $postData['mo2f_cron_hours'] * 60 *60;
        if($mo2f_cron_hours < 3600){
            do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('INVALID_HOURS'),'ERROR');
        }else{
    ...
    function mo_wpns_handle_file_backup_enable_form($postData){
             ...
             update_option( 'mo2f_enable_cron_file_backup', $enable );
                if(get_option('mo2f_enable_cron_file_backup') == '0'){
                	$handler_obj = new mo_site_backup;
                 	...
                 	do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('CRON_FILE_BACKUP_DISABLE'),'ERROR');
                 }
                else{
                 	do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('CRON_FILE_BACKUP_ENABLE'),'SUCCESS');
                 }
    }

    
include $mo_dirName . 'views'.DIRECTORY_SEPARATOR.'backup.php';