<?php
	...
	$template2	= "Hello ##username##,<br><br>Your account was logged in from new IP Address <b>##ipaddress##</b> on website <b>".get_bloginfo()."</b>. Please <a href='mailto:".mo_MoWpnsConstants::SUPPORT_EMAIL."'>contact us</a> if you don't recognise this activity.<br><br>Thanks,<br>".get_bloginfo();
	...
	if(current_user_can( 'manage_options' )  && isset($_POST['option']) )
	{
		switch($_POST['option'])
		{
			case "mo_wpns_enable_ip_blocked_email_to_admin":
				mo_wpns_handle_notify_admin_on_ip_block($_POST);						break;
			case "mo_wpns_enable_unusual_activity_email_to_user":
				mo_wpns_handle_notify_unusual_activity($_POST);						break;
			case "custom_user_template":
				mo_wpns_handle_custom_template($_POST['custom_user_template']);		break;
            		case "mo_wpns_get_manual_email"	:
			    	mo_wpns_handle_admin_email($_POST);                                    break; 
			case "custom_admin_template":
				mo_wpns_handle_custom_template(null,$_POST['custom_admin_template']);	break;
		}
	}
	...
	include $mo_dirName . 'views'.DIRECTORY_SEPARATOR.'notification-settings.php';



	/* ADMIN NOTIFICATION SETTINGS */
        function mo_wpns_handle_admin_email($postValue)
	{
		
        if(mo_validate_email($_POST['admin_email_address'])){
			...
			do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('EMAIL_SAVED'),'SUCCESS');
	    }else{
	    	do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('INVALID_EMAIL'),'ERROR');
	    }
	}
	function mo_validate_email($str) {
     return (!preg_match("/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix", $str)) ? FALSE : TRUE;
     }

	//Function to handle enabling and disabling of admin notification on ip blocking
	function mo_wpns_handle_notify_admin_on_ip_block($postValue)
	{
		...
		if($enable_ip_blocked_email_to_admin)
			do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('NOTIFY_ON_IP_BLOCKED'),'SUCCESS');
		else
			do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('DONOT_NOTIFY_ON_IP_BLOCKED'),'ERROR');
	}


	//Function to handle enabling and disabling of admin notification on unusual activity
	function mo_wpns_handle_notify_unusual_activity($postValue)
	{
		...
		if($enable_unusual_activity_email_to_user)
			do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('NOTIFY_ON_UNUSUAL_ACTIVITY'),'SUCCESS');
		else
			do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('DONOT_NOTIFY_ON_UNUSUAL_ACTIVITY'),'ERROR');
	}


	//Function to save unusual activity email template
	function mo_wpns_handle_custom_template($template1,$template2=null)
	{
		...
		do_action('mo_wpns_show_message',mo_MoWpnsMessages::showMessage('TEMPLATE_SAVED'),'SUCCESS');
	}